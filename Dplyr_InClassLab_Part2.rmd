...
Install the `"nycflights13"` package. Load (`library()`) the package. You'll also need to load `tidyverse` The data frame called `flights` should now be accessible to you.
...
my_flights <- mutate(my_flights, gain = dep_delay - arr_delay, speed = distance / air_time * 60, .before = 1)
```
...
```{r}
des_speed <- my_flights %>% arrange(desc(speed))

des_gain <- my_flights %>% arrange(desc(gain))
```

4.  If you haven't already, do the last 2 steps in a single statement using the pipe operator.

```{r}
my_flights_piped <- mutate(my_flights, gain = dep_delay - arr_delay, speed = distance / air_time * 60, .before = 1) %>% arrange(desc(speed))
```
...
```{r}
my_flights %>% filter(origin == "JFK" | origin == "HOU") %>% summarise(mean(arr_delay, na.rm = T))
```

Find the flights that are most delayed upon departure from each origin.

```{r}
my_flights %>% group_by(origin) %>% slice_max(dep_delay)
...
##giving me incomparable != F error, not sure why/ what to do
...
```{r}
starwars %>% group_by(homeworld) %>% summarise(n_characters = n()) %>% arrange(desc(n_characters))
```

Find the average height for each eye color. You might find some of the answers to be strange, but keep going!

```{r}
starwars %>% filter(eye_color == "brown") %>% summarise(mean_height = mean(height, na.rm = T))
...
starwars %>% filter(eye_color == "brown") %>% na.omit(height) %>% summarise(mean_height = mean(height))
```
